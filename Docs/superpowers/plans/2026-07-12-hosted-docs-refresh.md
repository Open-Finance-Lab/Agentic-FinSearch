...
- **F-6**: stale/incoherent phrases outside plan-named regions (final-review Minors): `usage/basic_usage.rst:34` Setting Button "for Advanced search" → Research mode; `introduction.rst:10` TradingView bullet lacks the crypto-only qualifier and omits the XBRL-taxonomy server; `project_structure.rst:113` Backend Highlights `mcp_server/` bullet omits XBRL.
- **F-7**: two clarity nits: `usage/memory_system.rst` API-Isolation bullet could cross-ref the /v1 statelessness note (per-request clear at `openai_views.py:273`); `api_reference.rst` News Signals "ETag/Last-Modified validators" — tickers-filtered variants are ETag-only (`signals_views.py` `_last_modified` returns None there).
