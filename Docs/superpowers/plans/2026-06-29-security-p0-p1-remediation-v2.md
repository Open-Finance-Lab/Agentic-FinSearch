...
**Tech Stack:** Django 6.0 (tests via `pytest`), `openai-agents` SDK, MCP (`mcp` SDK + npm MCP servers), `django_ratelimit`, Redis (`redis` py + `RedisCache`), gunicorn (gthread), Caddy edge, GitHub Actions, podman on the droplet, markdown-it + DOMPurify + KaTeX (Chrome extension frontend).
...
**Test convention:** This repo runs tests with **pytest** (there is no `pytest-django`; `tests/` is a namespace package with no `__init__.py`, so `manage.py test` discovers nothing — verified). `tests/conftest.py` configures Django once (`DJANGO_SETTINGS_MODULE=django_config.settings` + `django.setup()`) so `django.test.SimpleTestCase`/`RequestFactory`/`override_settings` work — **this bootstrap is a committed prerequisite of this plan** (Task 0). **Run every test step as `uv run pytest tests/<file>.py -v` from `Main/backend/`** (substitute pytest wherever a task says `manage.py test tests.<module>` — they are equivalent; the test files and test code in each task are correct as written). New tests use `django.test.SimpleTestCase` (no DB). The plan also flips CI `RUN_TESTS=true` (Task 1, pytest invocation) so this suite actually gates the deploy.
