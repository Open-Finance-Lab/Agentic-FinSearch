...
against the pinned signals-v2 schema and project onto ATL's 7-field
...
SCHEMA = REPO_ROOT / "Heartbeat" / "schemas" / "signals-v2.schema.json"
...
def test_fixture_supports_atl_projection():
    ...
    for entry in artifact["signals"].values():
        # sentiment/sentiment_score/headline/source/url/n_articles cross
        # directly; age_hours is derived consumer-side from published
        # (spec §4.5).
        for field in ("sentiment", "sentiment_score", "headline", "source",
                      "url", "published", "n_articles"):
            assert field in entry
        assert entry["sentiment"] in ("bullish", "bearish", "neutral")
        assert -1.0 <= entry["sentiment_score"] <= 1.0
